...
- You can use ARM templates, Bicep, or Terraform
...
- After deployment:
  ...
  - *DNS server*
    ...
    - You can use custom DNS servers
      - Affects all VMs
  ...
  - *Diagram*
    ...
    - Shows topology across subscriptions and regions

## Azure Virtual Network Manager (AVNM) - **NEW 2025**

- Provides **centralized management** for network connectivity and security rules across multiple VNets and subscriptions
- Can configure:
  - **Hub-and-spoke** connectivity topologies
  - **Mesh** connectivity between VNets
  - Deploy **NSG rules** at scale across multiple VNets
- Benefits:
  - Simplifies network management at scale
  - Consistent security policies across subscriptions
  - Centralized network architecture management
- Useful for enterprise-scale deployments with many subscriptions and VNets
...
## NAT Gateway - **UPDATED 2025**

### Standard NAT Gateway

- Provides secure outbound connectivity with static public IP
- Scales automatically
- Zonal (bound to specific availability zones)

### StandardV2 NAT Gateway (Preview)

- **Zone-redundant** - automatically spans across availability zones
- Supports both **IPv4 and IPv6**
- Provides up to **100 Gbps** of throughput
- Supports **flow logging** for better visibility
- Requires **StandardV2 public IP addresses**
- Cannot be upgraded from Standard NAT Gateway
- Recommended for **high-availability scenarios**

### Outbound Connectivity Changes - **IMPORTANT**

- **Microsoft will disable default outbound access for VMs by March 31, 2026**
- Deploying a **NAT Gateway** or other outbound method will be **mandatory**
- Plan migration to NAT Gateway before the deadline
- Outbound connectivity options:
  - NAT Gateway (recommended for production)
  - Public Load Balancer with outbound rules
  - Public IP addresses on VMs (not recommended for production)
- Augmented rules
