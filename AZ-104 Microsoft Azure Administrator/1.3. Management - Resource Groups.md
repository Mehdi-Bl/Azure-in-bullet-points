...
- Policies, role assignments, and locks assigned at a management group automatically apply to all nested subscriptions and resource groups
- This hierarchy is emphasized in the AZ-104 exam (2025)
- Tag inheritance can be applied at management group level (preview)
...
- ❗ All resources cannot be moved (some require manual recreation).
- Ways of moving
  - Using CLI: `az resource move --destination-group new-rg --id resourceid`
  - In portal: Overview → Move
- **Resource Mover** now supports cross-subscription and cross-region moves of most resource types
  - Enables migrations between Azure regions or reorganizing subscriptions without downtime
  - Some resource types still require manual recreation
