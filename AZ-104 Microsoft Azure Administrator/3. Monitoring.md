...
- Perform functions:
  ...
  - **Respond**: • Alerts • Autoscale • Alert processing rules (suppression/grouping)
  - **Integrate**: • Event Hubs • Logic Apps • Ingest & Export APIs • Grafana • Power BI
...
- Azure Monitoring Agents
  ...
  - **Azure Monitor Agent (AMA)** - **RECOMMENDED**
    - Replaces the legacy Log Analytics agent (retired Aug 31, 2024)
    - Better performance, enhanced security, and central configuration
    - Uses Data Collection Rules (DCRs) to configure data collection
    - Supports VMs, VM scale sets, and Azure VMSS flexible scale sets
    - DCRs can be reused across subscriptions for consistent configuration
  - **Log Analytics Agent** - **RETIRED**
    - No longer supported as of August 31, 2024
    - Data ingestion may fail, portal installation options removed
    - Customers should migrate to Azure Monitor Agent
