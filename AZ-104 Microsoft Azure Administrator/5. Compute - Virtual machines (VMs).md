...
- **Microsoft Monitoring Agent**
  - Onboards VM in Log Analytics (Migrate to Azure Monitor Agent)

## VM Security - **NEW 2025**

### Trusted Launch

- **Trusted Launch is now the default security baseline** for generation-2 VMs and scale sets
- Integrates secure boot and virtual TPM to protect:
  - Boot-loaders
  - Kernels
  - Drivers
- Safeguards secrets like keys and certificates
- Existing VMs can enable Trusted Launch through Azure portal or CLI
- Provides enhanced protection against boot-level attacks

### Confidential VMs

- Azure offers confidential VMs using **AMD SEV-SNP** and **Intel TDX** technology
- Provide **Trusted Execution Environments (TEEs)** that isolate:
  - Data and code from cloud operators
  - Data and code from other tenants
- VM series available:
  - **DCasv5**: General purpose confidential VMs
  - **DCadsv5**: Memory-optimized confidential VMs
  - **ECasv5**: Compute-optimized confidential VMs
- Confidential computing can be at:
  - **VM level** (IaaS) - full VM encryption
  - **Application level** - using enclaves
- Ideal for workloads requiring strong isolation (financial services, healthcare, government)
...
## Premium SSD v2 - **NEW 2025**

- Next generation of Azure Premium SSD
- Offers sub-millisecond latencies and flexible performance scaling
- Disk sizes: **1 GiB to 64 TiB**
- IOPS and throughput can be specified independently and scaled on the fly
- Up to **80,000 IOPS** and **1,200 MB/s** throughput
- Baseline: 3,000 IOPS and 125 MB/s per disk
- Supports features like:
  - **Live resize**: Scale disk size without downtime
  - **Instant snapshots**: Immediate use of snapshots for new disks
  - Cross-region restore capabilities
- More cost-effective than traditional Premium SSD
