# Hybrid Identities (Microsoft Entra ID)

- Hybrid (common) identity = Cloud + On Premises identity
- Connection is done through Azure AD Connect or Entra ID Cloud Sync
...
- **Common Identity**
  ...
  - **Passwordless Authentication** (Recommended)
    - FIDO2 security keys, Windows Hello for Business, Microsoft Authenticator app
    - Eliminates passwords and provides stronger security
  - **High Availability**
  - **Collaboration**
...
## Azure AD Connect & Entra ID Cloud Sync
...
- Manage in Entra ID → Connect → Synchronization Service
...
- **Azure AD Connect**: Traditional hybrid identity solution
- **Entra ID Cloud Sync**: Newer cloud-based synchronization solution (preview/GA depending on region)

### Preparing for Azure AD Connect

- Create a new user in Microsoft Entra ID as Global Administrator
- Download Azure AD Connect and install it
  - Need Windows Server 2016 or later (updated requirement)
...
- Installation settings
  ...
  - **Then**
    - **How users will sign-in**
      - One of them: *Password synchronization*, *Pass-through authentication*, *Federation with AD FS*, *Seamless SSO*
      - *Enable sign on* → Yes, No
    - **Forest and Entra credentials**
