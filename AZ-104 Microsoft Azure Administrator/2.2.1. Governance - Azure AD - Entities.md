# Microsoft Entra ID Entities (formerly Azure AD)

## Overview

- Entra ID entities are objects registered in Microsoft Entra ID.
- They're uniquely identifiable.

### Security principal

- Also known as **security identity** or **security object**.
- Defines the access policy and permissions for the user/application in the Entra ID tenant.
- Enables
...
- Member or Guest
  - Members are created within Entra ID directory
  - Guests are invited by administrator or other users of Entra ID
...
- Location is required if you want to assign license to a user within Entra ID
- Set usage location
  - In portal: Microsoft Entra ID → Users → Select User → Profile → Settings
- You can then assign license
  - In Portal: Microsoft Entra ID → Users → Select User → Licenses
- **User Principal name**: Combination of an user name + domain

### Create new user

- Microsoft Entra ID → User → new User
- User name
...
- You can add a user as an External User (Guest)
- Good for B2B collaboration scenarios
  - External users keep their identity in their home directory
...
- Manage in portal
  - Steps: Microsoft Entra ID → Password Reset
  - Configurations
    - **Enable**
...
- In Entra ID you can configure user access to application objects
...
- You manage in "Microsoft Entra ID → Groups"
...
- **Company Branding**
  - In portal: Microsoft Entra ID → Users and groups → Company branding
  - Allows you to customize the pages with e.g. banner, sign-in page text, user name hint

### Security vs Microsoft 365 Groups

- **Security groups**
  - Also known as **Entra ID security groups**
  - [Security principals](#security-principal) used for assigning permissions
  - Analogous to Security Groups in on-prem Windows Active Directory
- **Microsoft 365 Groups**
  - Also known as **Unified Groups**
  - Groups users to assign permissions to related resources (Teams, SharePoint, Outlook, etc.)
...
- For mobile devices and Windows 10/11
...
- Join device in both local AD and Entra ID
...
- Actual management is done through Group Policy or System Center Configuration Manager
  - They're tied in to Entra ID but not part of core AD
- Relies on AD Connect for synchronization
  - If they're already joined to local AD, they're also registered in Entra ID automatically
- Configuration
  - Ensure access to external Entra ID URLs
...
- Configurations
  ...
  - Additional administrators on Entra ID joined devices
...
### Entra ID Device Settings/Policies

- **Control permissions**
  - Who's allowed to access joined devices?
- **Control sync**
  - Enabled/disabled
...
- Entra ID IDaaS (Identity Directory as a Service)
- Application types
...
- Automated user provisioning through SCIM 2.0
  ...
  - SCIM
    ...
    - Defined by IETF
    - Controls users, groups and their relations
  - Available on select SaaS apps
- In portal, you can assign access to applications
  - Microsoft Entra ID → Enterprise applications → Select application → Users and groups
...
- Object is globally unique with ID known as **app ID** or **client ID**
...
- [Security principal](#security-principal) enabling access to Entra ID secured resources
- Defines the access policy and permissions for the user/application in the Entra ID tenant
- There are three types of service principal:
  ...
  2. Managed identity
     ...
     - Can be granted access and permissions
     - ❗️ They cannot be updated or modified directly
  3. Legacy
     - Created before app registrations were introduced or using legacy ways
- On portal they can be managed in "Microsoft Entra ID → Enterprise Applications" blade
...
- In Azure portal, service principals are listed and can be managed in "Microsoft Entra ID → Enterprise Applications" blade
- Enterprise Applications is category of Service Principal where
  ...
  - Service Principal has tag `WindowsAzureActiveDirectoryIntegratedApp`
    - ❗️ Otherwise it won't be listed in "Microsoft Entra ID → Enterprise Applications"
...
- Tenancy
  ...
  - *Service principal* is the local representation for use in a specific tenant
    - Must be created in each tenant where the application is used
