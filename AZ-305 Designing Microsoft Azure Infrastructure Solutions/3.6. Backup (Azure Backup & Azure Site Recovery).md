...
- Data is stored in Recovery Services Vault
- **Enhanced 2025 Features**:

### Vaulted Backup for Premium File Shares (2025)
- **10-Year Retention**: Long-term retention up to 10 years for premium file shares
- **Offsite Copy**: Automatic offsite backup copy
- **Immutable Storage**: Protection against ransomware and accidental deletion
- **Use Cases**: Compliance, long-term retention, audit requirements

### Cross-Region Restore (2025)
- **Feature**: Restore to alternate region (paired region)
- **Cross-Subscription**: Restore to different subscription
- **Use Cases**: Regional outage recovery, disaster recovery planning
- **Benefits**: Proactive disaster recovery, multi-subscription support

### Immutable Vaults (2025)
- **WORM**: Write Once, Read Many storage model
- **Ransomware Protection**: Immutable against ransomware attacks
- **Compliance**: Meets regulatory requirements (SOX, HIPAA, PCI-DSS)
- **Configuration**: Immutable policy settings
- **Benefits**: Maximum protection for critical backup data

### Azure Backup Center (2025)
- **Centralized Management**: Unified backup management dashboard
- **Compliance**: Track backup compliance across all workloads
- **Reporting**: Comprehensive backup reports and insights
- **Cost Optimization**: Identify cost savings opportunities
- **Alerts**: Unified alerting for all backup operations

### Enhanced VM Backup
- **Instant Restore**: Fast VM restoration from storage snapshots
- **Cross-Region Backup**: Backup to secondary region
- **Application-Consistent**: Application-aware backups for Windows and Linux
- **Enhanced Security**: Encryption at rest and in transit

## Azure Site Recovery (2025)

- Built to provide a data center disaster recovery solution for VM workloads to Azure
- Recovers from:
  - Hyper-V
  - VMware
  - Physical hosts
...
#### Cross-Subscription Disaster Recovery (Preview)
- **Feature**: Replicate VMs across subscriptions
- **Use Cases**: Cloud migration, subscription consolidation
- **Benefits**: Flexible DR strategies, subscription management

#### Enhanced Replication
- **Bandwidth Optimization**: Improved bandwidth efficiency
- **Compression**: Automatic data compression
- **Throttling**: Configurable bandwidth throttling

#### Disaster Recovery Planning
- **Recovery Plans**: Orchestrated failover with dependencies
- **Azure Automation**: Runbook automation for complex recovery
- **Test Failover**: Non-disruptive DR testing
- **Failback**: Automated failback to on-premises
