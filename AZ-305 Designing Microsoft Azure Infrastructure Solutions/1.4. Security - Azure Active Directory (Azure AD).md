...
**Important**: Azure Active Directory has been rebranded as **Microsoft Entra ID** in 2025. While the core concepts remain the same, terminology has evolved. This document covers both legacy Azure AD and current Microsoft Entra ID capabilities.

# Microsoft Entra ID

- **Formerly**: Azure Active Directory (Azure AD)
- **New Name**: Microsoft Entra ID (effective 2025)
...
## 2025 Key Updates and Features

### Microsoft Entra External ID with Custom OIDC Identity Provider Federation
- **Custom OIDC Providers**: Users can sign in with accounts from other identity providers
- **External Identities**: Collaborate with users outside your organization
- **Federation Support**: Support for custom OIDC identity providers
- **Benefits**: Simplify external user access without creating separate accounts

### Advanced Identity Governance
- **Entitlement Management**: Access packages for resources
- **Access Reviews**: Regular certification of access rights
- **Privileged Identity Management**: Enhanced PIM with advanced controls
- **Terms of Use**: Conditional access policies with terms acceptance

### Conditional Access (Enhanced)
- **Risk-Based Policies**: ML-based detection of risky sign-ins
- **Location-Based Access**: Restrict access by geographic location
- **Device-Based Policies**: Require compliant or hybrid-joined devices
- **Application-Based Controls**: Block or grant access to specific apps
- **Continuous Access Evaluation**: Real-time token validation

### Multi-Factor Authentication (MFA) - 2025
- **Passwordless Authentication**: FIDO2, Windows Hello, Microsoft Authenticator
- **Zero Trust Integration**: MFA as part of Zero Trust strategy
- **Continuous Authentication**: Adaptive MFA based on risk
